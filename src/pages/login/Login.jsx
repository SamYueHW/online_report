...
const Login = () => {
  ...
  const handleSignUpClick = async(e) => {
    ...
    if (isSignUpMode) {
      ...
      // 发送加密后的密码到服务器端
        const response = await axios.post('http://localhost:3001/register', { email: email, password: password, activationKey: key }, {
        });
         // 处理登录成功的情况
        // 检查响应
  ...
  const handleSignInClick = async (e) => {
    ...
    if (!isSignUpMode) {
      ...
      // 发送加密后的密码到服务器端
        const response = await axios.post('http://localhost:3001/login', { email: email, password: password}, { withCredentials: true });
        // 处理登录成功的情况
        if (response.status === 200) {
